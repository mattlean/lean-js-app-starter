...
`testPathIgnorePatterns` will have Jest ignore the `node_modules/` (which is ignored by default) and `build/` directories as well as [`src/config/test.js`](../../../../src/config/test.js) and [`src/util/test.js`](../../../../src/util/test.js) because these files are not tests and by default Jest looks for any filename that includes `test.js`.
