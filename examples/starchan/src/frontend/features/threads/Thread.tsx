    ...
    if (data.replyCount && data.replyCount > 5 && data.replies) {
      ...
      omittedTxt = (
        <p className="omitted-txt">
          {omittedCount} {omittedCount === 1 ? "reply" : "replies"} omitted.{" "}
          <Link to={`/thread/${data.id}`}>Click here</Link> to view.
        </p>
      );
    }
  }
    ...
    <>
      <section>
        <header id={data.id}>
          ...
          <time dateTime={serverCreatedAt.toISOString()}>{createdAt}</time>{" "}
          <Link to={`/thread/${data.id}#${data.id}`} className="jump-to-link">
            Id.{data.id}
          </Link>
          {replyLink}
        </header>
