...
describe("create thread endpoint", () => {
  ...
  it("creates thread with subject & comment when request payload has subject & comment", async () => {
    prismaMock.thread.create.mockResolvedValue(MOCK_THREAD_W_SUBJECT_COMMENT);
    // Prisma typing is incorrect here
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    prismaMock.thread.aggregate.mockResolvedValue({ _count: 1 });
    ...
    expect(res.body.data.subject).toBe(MOCK_THREAD_W_SUBJECT_COMMENT.subject);
    expect(res.body.data.comment).toBe(MOCK_THREAD_W_SUBJECT_COMMENT.comment);
    expect(new Date(res.body.data.createdAt).getTime()).toBe(
  ...
  it("deletes thread when creating a thread while the threads list is full", async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    prismaMock.thread.aggregateRaw.mockResolvedValue([MOCK_THREAD_LIST_RES[0]]);
...
describe("read thread endpoint", () => {
  it("returns thread", async () => {
    ...
    expect(MOCK_THREAD_W_SUBJECT_COMMENT.subject).toBe(res.body.data.subject);
    expect(MOCK_THREAD_W_SUBJECT_COMMENT.comment).toBe(res.body.data.comment);
  ...
  it("returns 404 when requesting a nonexistent thread", async () => {
    prismaMock.thread.findUniqueOrThrow.mockImplementation(() => {
      const err = new Prisma.PrismaClientKnownRequestError("No thread found", {
...
describe("create reply endpoint", () => {
  it("creates reply for thread", async () => {
    prismaMock.thread.findUniqueOrThrow.mockResolvedValue(MOCK_THREAD_W_REPLY);
