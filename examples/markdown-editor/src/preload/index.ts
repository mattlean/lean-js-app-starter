...
const API = {
  ...
  onColorModeMenu: (callback: (colorMode: colorModes) => void) => {
    const listener = (_: Electron.IpcRendererEvent, colorMode: colorModes) => {
      callback(colorMode);
    };
  ...
  onMainErrorMessage: (callback: (errorMessage: string) => void) => {
    const listener = (_: Electron.IpcRendererEvent, errorMessage: string) => {
  ...
  onMainHtmlExportDialog: (callback: () => void) => {
    ...
    return () => ipcRenderer.removeListener("mainhtmlexportdialog", listener);
  },
  ...
  onMainMdOpenDialog: (callback: () => void) => {
    ...
    return () => ipcRenderer.removeListener("mainmarkdownopendialog", listener);
  },
  ...
  onMainMdOpenRecent: (callback: (recentFilePath: string) => void) => {
    const listener = (_: Electron.IpcRendererEvent, recentFilePath: string) => {
      callback(recentFilePath);
    };

    ipcRenderer.on("mainmarkdownopenrecent", listener);

    return () => ipcRenderer.removeListener("mainmarkdownopenrecent", listener);
  },
  ...
  onMainMdSave: (callback: (exitType?: exitTypes) => void) => {
    const listener = (_: Electron.IpcRendererEvent, exitType?: exitTypes) => {
      callback(exitType);
    };
