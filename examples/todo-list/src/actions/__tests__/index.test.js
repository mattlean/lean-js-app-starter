...
describe('actions', () => {
    ...
    test(`fetchTodosFailure('${filter}', '${message}') creates an action that ends todos fetch request with failure`, () => {
        ...
        expect(actions.fetchTodosFailure(filter, message)).toEqual(
            expectedAction,
        )
    })
    ...
    test(`fetchTodosSuccess('${filter}', {OBJECT}) creates an action that ends todos fetch request successfully`, () => {
        ...
        expect(actions.fetchTodosSuccess(filter, mockDatabase.todos)).toEqual(
            expectedAction,
        )
    })
    ...
    test('toggleTodo({ID}) creates an action to toggle a todo', () => {
        ...
        return store
            .dispatch(actions.toggleTodo(mockDatabase.todos[0].id))
            .then(() => {
                expect(store.getActions()).toEqual(expectedActions)
            })
    })
